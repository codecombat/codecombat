...
module.exports =
  ...
  postPatch: wrap (req, res) ->
    ...
    reasonNotToAutoAccept = undefined

    validation = tv4.validateMultiple(changedCourse, Course.jsonSchema)
    if not validation.valid
      reasonNotToAutoAccept = 'Did not pass json schema.'
    else if not isJustFillingTranslations(normalizedDelta)
      reasonNotToAutoAccept = 'Adding to existing translations.'
    else
    ...
    patch.set({
      ...
      status: if reasonNotToAutoAccept then 'pending' else 'accepted'
      created: new Date().toISOString()
      reasonNotToAutoAccept
    })
    database.validateDoc(patch)

    if reasonNotToAutoAccept
