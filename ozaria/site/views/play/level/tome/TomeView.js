...
const template = require('app/templates/play/level/tome/tome.pug');
  ...
  TomeView = class TomeView extends CocoView {
    ...
    constructor(options) {
      ...
      this.unwatchFn = store.watch(
        ...
        solution => this.onChangeMyCode(solution.source)
      );
    ...
    afterRender() {
      ...
      this.castButton = this.insertSubView(new CastButtonView({spells: this.spells, level: this.options.level, session: this.options.session, god: this.options.god, capstoneStage: this.options.capstoneStage}));
    ...
    onCommentMyCode(e) {
      for (var spellKey in this.spells) {
        ...
        if (spell.canWrite()) {var commentedSource;
        
          if ((__guard__(spell != null ? spell.level : undefined, x => x.get('ozariaType')) === 'capstone') && this.options.session) {
            var comment = spell.view.getLanguageComment();
            var sessionCommentedCode = (__guard__(this.options.session.attributes != null ? this.options.session.attributes.code["hero-placeholder"] : undefined, x1 => x1.plan) || "")
              .split('\n')
              .map(line => comment + ' ' + line)
              .join('\n');

            this.options.session.attributes.code["hero-placeholder"].plan = sessionCommentedCode;
    ...
    createSpells(programmableThangs, world) {
      let left, left1, spell, spellKey, thang;
    ...
    onSpellLoaded(e) {
      ...
      return this.cast(false, false, justBegin, false, true);
    }
    ...
    cast(preload, realTime, justBegin, cinematic, spellJustLoaded) {
      var left, left1;
      ...
      if (spellJustLoaded == null) { spellJustLoaded = false; }
    ...
    onSpriteSelected(e) {
      let needle;
      if (this.spellView && (needle = this.options.level.get('type', true), ['hero', 'hero-ladder', 'hero-coop', 'course', 'course-ladder', 'game-dev', 'web-dev'].includes(needle))) { return; }  // Never deselect the hero in the Tome.
    ...
    reloadAllCode() {
      ...
      for (var spellKey in this.spells) {
        var spell = this.spells[spellKey];
        if (spell.view && ((spell.team === me.team) || (['common', 'neutral', null].includes(spell.team)))) {
          var maxStage = Math.max(...Array.from((this.options.level.get('additionalGoals') || []).map(g => g.stage) || []));
          if ((this.options.level.get('ozariaType') === 'capstone') && ((this.options != null ? this.options.capstoneStage : undefined) === maxStage) && (this.options.level.get('creativeMode') === true)) {
            ...
            var priorStructuredCode = __guard__(this.options.session.get('code')['saved-capstone-normal-code'], x1 => x1.plan);
    ...
    softReloadCapstoneStage(newCapstoneStage) {
      return this.castButton.softReloadCapstoneStage(newCapstoneStage);
    }
