    ...
    methods: {
      ...
      begin () {
        ...
        this.$nextTick(() => {
          ...
          // NOTE: Looping over and pushing into an array here to retain the indexes as ID's even when we
          // filter out specific steps for the non admin feature flag.
          // Can be replaced with a simple .map when we drop the feature flag in the future.
          const steps = []
          this.tutorialSteps.forEach((tutorialStep, index) => {
            // NOTE: We allow all messages for admins. Only intros and old school stationary messages for regular users
            // until game designers have gone back through older levels and updated the tutorial messages
            const onlyStationary = this.isStepOnlyStationary(tutorialStep)
            if (!me.isAdmin() && !tutorialStep.intro && !onlyStationary) {
              return
            }
            ...
            if (index === 0) {
              ...
              if (!unusualLastStep) {
                ...
                // We manage this by adding one extra stationary step. Because of this extra final step,
                // we need to a nextButton to get to it:
          ...
          if (!this.restartAtId) {
            ...
            // If we have had the steps modified, we need to jump directly to a specific step
            const realSteps = steps.filter(s => s.text !== this.defaultStep.message)
            if (realSteps.length !== this.previousLength) {
              this.tour.show(realSteps[realSteps.length - 1].id)
            }

            this.previousLength = null
          }
        })
      },
      ...
      delayedRenderTrigger: function (step, tutorialStep, alreadySeen) {
        setTimeout(() => {
          ...
          if (step.options.id === 0) {
            ...
            footerElement.append(`<div class="shepherd-footer-learning-goals-title">${this.$t('play_level.learning_goals')}: </div>`)
            footerElement.append(`<div class="shepherd-footer-learning-goals-description" >${tutorialStep.intro.learningGoals}</div>`)
          ...
          if (attachTo.on === 'right') {
            ...
            headerElement.addClass('shepherd-header-right')
          ...
          if (tutorialStep.targetElement === 'Code Bank Button') {
            $(attachTo.targetElement).addClass('golden-highlight-outline')
          }

          if (tutorialStep.animation === 'Glow') {
            $(attachTo.targetElement).addClass('button-glow')
          }
