...
describe 'GET /db/classroom/:handle/member-sessions', ->
  ...
  beforeEach utils.wrap (done) ->
    ...
    @session1A = yield new LevelSession({creator: @student1.id, state: { complete: true }, level: {original: @levelA._id.toHexString()}, permissions: [{target: @student1._id, access: 'owner'}]}).save()
    @session1B = yield new LevelSession({creator: @student1.id, state: { complete: false }, level: {original: @levelB._id.toHexString()}, permissions: [{target: @student1._id, access: 'owner'}]}).save()
    @session2A = yield new LevelSession({creator: @student2.id, state: { complete: true }, level: {original: @levelA._id.toHexString()}, permissions: [{target: @student2._id, access: 'owner'}]}).save()
    @session2B = yield new LevelSession({creator: @student2.id, state: { complete: false }, level: {original: @levelB._id.toHexString()}, permissions: [{target: @student2._id, access: 'owner'}]}).save()
    yield utils.loginUser(@teacher)
