(function () {
  ...
  function renderGrid (rows, heroX, heroY, form) {
    ...
    for (let y = 0; y < rows.length; y++) {
      for (let x = 0; x < rows[y].length; x++) {
        ...
        const formClass = isHero ? (form === 'frog' ? ' form-frog' : ' form-wizard') : ''
        tile.className = 'tile ' + visual.className + (isHero ? ' hero' : '') + formClass
        tile.textContent = visual.text
        tile.setAttribute('role', 'gridcell')
        tile.setAttribute('aria-label', (isHero ? (form === 'frog' ? 'カエルの姿のヒーロー、' : 'ヒーロー、') : '') + visual.label)
  ...
  function completeMission (mission) {
    ...
    setFeedback(isFinal ? '全21ミッション、クリア！ JavaScript マスターへの第一歩です！ 🎉' : 'ミッションクリア！ すべてのマップでコードが成功しました。 🎉', 'success')
