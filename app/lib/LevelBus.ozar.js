...
module.exports = (LevelBus = (function() {
  LevelBus = class LevelBus extends Bus {
    ...
    onSpellCreated(e) {
      if (!this.onPoint()) { return; }
      const spellTeam = e.spell.team;
      if (this.teamSpellMap == null) { this.teamSpellMap = {}; }
      if (this.teamSpellMap[spellTeam] == null) { this.teamSpellMap[spellTeam] = []; }

      if (!Array.from(this.teamSpellMap[spellTeam]).includes(e.spell.spellKey)) {
        this.teamSpellMap[spellTeam].push(e.spell.spellKey);
      }
      this.changedSessionProperties.teamSpells = true;
      this.session.set({'teamSpells': this.teamSpellMap});
      this.saveSession();
    ...
    onWinnabilityUpdated(e) {
      ...
      if ((needle = e.level.get('slug'), !['ace-of-coders', 'elemental-wars', 'the-battle-of-sky-span', 'tesla-tesoro', 'escort-duty', 'treasure-games', 'king-of-the-hill'].includes(needle))) { return; }  // Mirror matches don't otherwise show victory, so we win here.  # TODO: remove once these levels are configured as mirror matches
    ...
    onVictory(e) {
      if (!this.onPoint()) { return; }
      if (e && e.capstoneInProgress) { return; }
      const state = this.session.get('state');
