...
module.exports = {
    exportStudentProgress({classroom, sortedCourses, students, courses, courseInstances, levels, progressData}){
      ...
      const courseIds = ((() => {
        const result = [];
        for (c of Array.from(sortedCourses)) {           result.push(courses.get(c._id));
        }
        return result;
      })());
      ...
      for (var student of Array.from(students.models)) {
        var courseID, level;
        var concepts = [];
        for (trimCourse of Array.from(classroom.getSortedCourses())) {
          ...
          if (instance && instance.hasMember(student)) {
            for (trimLevel of Array.from(trimCourse.levels)) {
              ...
              var progress = progressData.get({ classroom: classroom, course, level, user: student });
