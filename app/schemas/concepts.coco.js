...
const concepts = [
  ...
  {concept: 'arithmetic', name: 'Arithmetic', description: 'Adding, subtracting, multiplication or dividing.', automatic: true, tagger(ast) { return _.some(ast.find('BinaryExpression'), n => ['+','-','*','/'].includes(n.operator) && ((n.right != null ? n.right.value : undefined) !== 1)); }},
  ...
  {concept: 'strings', name: 'Strings', description: 'Using string literals.', automatic: 'Yes, assuming this means string literals.', tagger(ast) { return _.some(ast.find("Literal"), e => typeof e.value === 'string'); }},
  ...
  {concept: 'xy_coordinates', name: 'Coordinate Pairs', description: 'This level uses XY coordinates to navigate: any time we use a method .*XY or use variables or properties named x or y', automatic: true, tagger(ast) {
      if (!(ast.find('MemberExpression.object>Identifier[name="y"]').length > 0)) { return false; }
      if (!(ast.find('MemberExpression.object>Identifier[name="x"]').length > 0)) { return false; }
      return true;
    }},
