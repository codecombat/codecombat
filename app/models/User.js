...
export default User = (function() {
  ...
  User = class User extends CocoModel {
    ...
    getTTSExperimentValue() {
      let value = {true: 'beta', false: 'control', control: 'control', beta: 'beta'}[utils.getQueryVariable('tts')];
      if (value == null) { value = me.getExperimentValue('tts', null, 'beta'); }
      if ((value == null) && (typeof features !== 'undefined' && features !== null ? features.china : undefined)) {
      ...
      if ((value == null) && __guard__(me.get('stats'), x => x.gamesCompleted)) {
        // Don't include players who have already started playing; just let them use it
        value = 'beta';
      }
      if ((value == null) && (new Date(me.get('dateCreated')) < new Date('2023-02-09'))) {
        // Don't include users created before experiment start date; just let them use it
        value = 'beta';
      }
      if ((value == null)) {
        let valueProbability;
        const probability = __guard__(__guard__(window.serverConfig != null ? window.serverConfig.experimentProbabilities : undefined, x2 => x2.tts), x1 => x1.beta) != null ? __guard__(__guard__(window.serverConfig != null ? window.serverConfig.experimentProbabilities : undefined, x2 => x2.tts), x1 => x1.beta) : 0.5;
