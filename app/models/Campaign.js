...
export default Campaign = (function() {
  Campaign = class Campaign extends CocoModel {
    ...
    static fetchIntroContentDataForLevels(campaignLevelsModuleMap) {
      let levels;
      let introLevels = [];
      for (var campaignId in campaignLevelsModuleMap) {
        var campaignModules = campaignLevelsModuleMap[campaignId];
        for (var moduleNum in campaignModules.modules) {
    ...
    static getLevelNumberMap(campaign, forceCourseNumbering) {
      const levels = [];
      for (var level of Array.from(this.getLevels(campaign))) {
        if (!level.original) { continue; }
        var practice = this.levelIsPractice(level, (campaign.type === 'course') || forceCourseNumbering);
        var assessment = this.levelIsAssessment(level);
