  ...
  LevelComponent = class LevelComponent extends CocoModel {
    ...
    getDependencies(allComponents) {
      ...
      for (var dep of Array.from(this.get('dependencies') || [])) {
        var comp = _.find(allComponents, c => (c.get('original') === dep.original) && (c.get('version').major === dep.majorVersion));
        for (var result of Array.from(comp.getDependencies(allComponents).concat([comp]))) {
