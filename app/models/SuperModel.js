...
module.exports = (class SuperModel extends Backbone.Model {
  ...
  report () {
    ...
    const unfinished = []
    for (const resource of Array.from(_.values(this.resources))) {
      if (resource) {
  ...
  loadCollection (collection, name, fetchOptions, value) {
    ...
    const cachedCollection = this.collections[url]
    if (cachedCollection) {
      ...
      const onCollectionSynced = function (c) {
  ...
  getModel (ModelClassOrUrl, id) {
    if (_.isString(ModelClassOrUrl)) { return this.getModelByURL(ModelClassOrUrl) }
    const m = new ModelClassOrUrl({ _id: id })
  ...
  registerModel (model) {
    this.models[model.getURL()] = model
  }
