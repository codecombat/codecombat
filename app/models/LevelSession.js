...
module.exports = (LevelSession = (function () {
  LevelSession = class LevelSession extends CocoModel {
    ...
    readyToRank () {
      ...
      for (const item of Array.from(thangSpellArr)) {
        const thang = item[0]
        const spell = item[1]
    ...
    recordScores (scores, level) {
      ...
      const newTopScores = []
      const now = new Date()
      for (let scoreType of Array.from((left = level.get('scoreTypes')) != null ? left : [])) {
        if (scoreType.type) { scoreType = scoreType.type }
        const oldTopScore = _.find(oldTopScores, { type: scoreType })
        let newScore = scores[scoreType]
        if (newScore == null) {
    ...
    static getTopScores ({ level, session }) {
      ...
      const scores = ((() => {
        const result = []
        for (score of Array.from((session.state != null ? session.state.topScores : undefined) != null ? (session.state != null ? session.state.topScores : undefined) : [])) {
          result.push(_.clone(score))
        }
        return result
      })())
      ...
      if (level) {
        for (const sessionScore of Array.from(scores)) {
          const thresholdAchieved = Level.thresholdForScore(_.assign(_.pick(sessionScore, 'score', 'type'), { level }))
    ...
    inLeague (leagueId) {
      ...
      for (const league of Array.from(this.get('leagues'))) {
    ...
    saveKeyValueDb () {
      ...
      for (const key in keyValueDb) {
        const value = keyValueDb[key]
        const oldValue = this.originalKeyValueDb[key]
