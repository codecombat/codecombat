...
module.exports = (Classroom = (function () {
  Classroom = class Classroom extends CocoModel {
    ...
    getLevelsByModules () {
      ...
      for (const course of Array.from(this.get('courses'))) {
        const courseLevels = this.getLevels({ courseID: course._id }).models
        const capstoneLevel = courseLevels.find(l => l.isCapstone())
        const isCh1 = course._id === utils.courseIDs.CHAPTER_ONE
    ...
    fetchIntroContentDataForLevels (courseModuleLevelsMap) {
      ...
      let introLevels = []
      for (const course of Array.from(this.get('courses'))) {
        for (const moduleNum in courseModuleLevelsMap[course._id].modules) {
    ...
    statsForSessions (sessions, courseID, levelsCollection) {
      let complete, left, nextLevel, nextLevelNumber
      ...
      for (let index = 0; index < courseLevels.models.length; index++) {
        ...
        complete = false
        const session = levelSessionMap[level.get('original')]
        if (session) {
          let left1
    ...
    statsForAIProjects (sessions, courseId) {
      ...
      const projects = sessions.models.map(p => p.toJSON())
      const projectsByScenario = _.groupBy(projects, 'scenario')
