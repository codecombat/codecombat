module.exports = {
  ...
  translation: {
    ...
    esper: {
      line_no: '看看本鸭发现了什么！第$1行有情况：',
      uncaught: '嘎！有个错误偷偷溜进来，但还是被本鸭的“火眼金睛”逮住了：$1',
      reference_error: '引用翻车了，别慌，本鸭带你做！',
      argument_error: '参数开始摆烂了，咱给它正正骨！',
      type_error: '拼写错误，手滑了？尊嘟假嘟？快检查一下！',
      syntax_error: '语法写得像外星文，本鸭差点没绷住，快改改！',
      error: '出小差啦，不过没关系，鸭鸭陪你debug～',
      x_not_a_function: '`$1` 只是个老实巴交的变量，不要让它当函数呀！',
      x_not_defined: '`$1` 还没定义就想用？先声明再说。',
      spelling_issues: '拼写翻车啦：你想说的是 `$1`，不是 `$2`，对吧？',
      capitalization_issues: '快醒醒！大小写搞错了：`$1` 应该写成 `$2`？',
      py_empty_block: '`$1` 里面空落落的？在 `$2` 后面的语句前加4个空格缩进呀。',
      fx_missing_paren: '想召唤 `$1` 当函数？后面加个 `()` 它就听话啦，信本鸭！',
      unmatched_token: '`$1` 落单了！每个 `$2` 都需要一个 `$3` 配对才完整哟。',
      unterminated_string: '字符串没结尾，末尾再加一个 `"` 收工~',
      missing_semicolon: '嘎，分号被吃掉了？快补上 `;`。',
      missing_quotes: '嘎！引号离家出走了，试试 `$1`。',
      argument_type: '`$1` 的参数 `$2` 要 `$3` 类型，你却塞了个 `$4`：`$5`？！快改快改！',
      argument_type2: '`$1` 的参数 `$2` 需要 `$3` 类型，现在是个 `$4`，专心一点呦。',
      target_a_unit: '攻击一个目标鸭，别光看戏吃瓜啦！快动手～',
      attack_capitalization: '攻击 `$1`，不是 `$2`！大小写很重要，别让它们互换身份鸭！',
      empty_while: 'while 循环里空荡荡？在里面写代码，前面加4个空格。',
      line_of_site: '`$1` 的参数 `$2` 有问题，你视线里有敌人吗？别盯着空气输出鸭！',
      need_a_after_while: '`$2` 后面漏了 `$1`，赶紧补上。',
      too_much_indentation: '缩进都快溢出屏幕了！往左挪挪，给其他代码留点位置～',
      missing_hero: '缺了 `$1` 关键字，要写 `$2` 才对。',
      takes_no_arguments: '`$1` 不接受参数，别硬塞给它，它会自闭的！',
      no_one_named: '没有叫 "$1" 的目标，是不是记错名字了？',
      separated_by_comma: '函数调用参数要用逗号隔开，`,` 别忘了。',
      protected_property: '动不得！受保护的属性：$1，别瞎碰。',
      need_parens_to_call: '想把 `$1` 当函数调用？后面加上 `()` 吧，不加怎么会。',
      expected_an_identifier: "这里应该是个标识符，结果看到 '$1'，什么鬼。",
      unexpected_identifier: '嘎！冒出来个不该出现的标识符。',
      unexpected_end_of: '输入突然没了？本鸭还在津津有味地读呢，快把后半段补上！',
      unnecessary_semicolon: '分号多余啦，删掉删掉。',
      unexpected_token_expected: '标记不对哦，这里想要 $1，解析 $3 时却撞见 $2，蚌埠住了。',
      unexpected_token: '居然冒出个 $1，本鸭都惊呆了，意想不到的剧情！',
      unexpected_token2: '你也在搞抽象吗？意外标记，这什么鬼？',
      unexpected_number: '数字来得不是时候，快让它排到后面去！',
      unexpected: "突然冒出来个 '$1'，你是不是在搞偷袭？",
      escape_pressed_code: 'ESC键按了？代码跑路，鸭鸭下班。',
      target_an_enemy: '瞄准敌人名字，像 `$1`，而不是字符串 `$2`，嘎！',
      target_an_enemy_2: '攻击要用敌人名字，比如 $1，不然敌人都笑场咯~',
      cannot_read_property: "无法读取未定义的属性 '$1'，读了个寂寞。",
      attempted_to_assign: '想改只读属性？想得美，但你可以换个思路～',
      unexpected_early_end: '程序提前摆烂了？别急，检查一下是不是哪里少写了结尾？',
      you_need_a_string: '构建需要字符串，从 $1 里挑一个吧。',
      unable_to_get_property: "拿不到属性 '$1'？因为它像本鸭的腹肌一样虚无～试试换个名字？",
      code_never_finished_its: '代码跑得比树懒还慢，要么卡住了，要么掉进死循环了！嘎！快救救它！',
      unclosed_string: '字符串门没关，漏风了。',
      unmatched: "不匹配的 '$1'，像一只落单的袜子，快给它找个伴儿！",
      error_you_said_achoo: '你说的密码是 $1，实际是 $2，大小写很关键哦。',
      indentation_error_unindent_does: '缩进错乱！取消缩进没对齐任何外层，检查空格。',
      indentation_error: '缩进出问题了。',
      need_a_on_the: '在 `$1` 后面加上冒号 `:`，别忘。',
      attempt_to_call_undefined: "想调用 '$1'？它是个空值，醒醒。",
      unterminated: '`$1` 没有闭合，还敞着呢。',
      target_an_enemy_variable: '用变量 `$1` 攻击，别用字符串 `$2`。试试 `$3`。',
      error_use_the_variable: '用变量名 `$1`，别用字符串 `$2`，鸭鸭拜托。',
      indentation_unindent_does_not: '缩进取消没对齐外层，用空格排排队！',
      unclosed_paren_in_function_arguments: '函数参数里的 $1 没关上，漏风咯。',
      ...
      error_index_empty: '未能读取 `__specTypeName__.__name__`，空的？',
      error_call_non_function: '`__name__` 不是函数，别当函数使。',
      error_not_iterable: '`__var__` 不可遍历，它不配。',
      error_assignment_invalid: '赋值引用无效，这啥操作？',
      error_invalid_array_length: '数组长度不对劲，鸭鸭皱眉。',
      error_didnt_make_generator: '`__name__` 可不是生成器，别指望它 yield。',
      error_undefined_variable: '`__name__` 还没定义就想用？嘎。',
      error_declared_identifier: '`__name__` 已经被占用，换一个名字。',
      error_declarations_cant_resolve: '解析不了声明组件 `__type__`，这组件成谜了。',
      error_strict_no_statement: '严格模式禁止 with 语句，别耍花招。',
      error_cant_resolve_ref_component: '`__type__` 引用组件解析失败，查无此物。',
      error_cant_convert_null: '无法将 undefined 或 null 转换为对象，它们不配。',
      error_need_object: '这里得要个对象，别给空。',
      error_illegal_object_prototype: '鸭鸭提醒你，对象原型只能是 Object 或 null。',
      error_no_prototype: '没有原型，继承链断了。',
      error_cant_write_property_to_undefined: 'undefined 上哪有属性 `__idx__`，改不了。',
      error_cant_write_property_to_non_obj: '非对象没有属性 `__idx__`，省省吧。',
      error_function_isnt_constructor: '嘎！这函数当不了构造函数，别 new 它。',
      error_write_protected_property: '属性 `__name__` 受保护，动不得。',
      error_read_protected_property: '受保护的属性 `__name__` 不让你看。',
      error_convert_value_failed: '`__orig__` 转不成 `__new__`，类型八字不合。',
      error_convert_object_to_primitive: '对象变不成原始值，强行转型失败。',
      error_first_argument_to_be_function: '`__fun__` 的第一个参数得是个函数，你传的啥？',
      error_write_non_writable: '这值铁了心不让改。',
      error_delete_nonconfigurable_object: '这对象不可配置，别想删。',
      error_cant_access_get: '拿不到 `__name__`，可能被藏起来了。',
      error_base_obj_not_invokable: '对象不是函数，别当函数用。',
      error_arg2_not_callable: '第二个参数得是函数，你乱塞。',
      error_reduce_empty_array_without_init: '空数组 reduce 没有初始值，嘎。',
      error_realm_prototype_not_realm: 'Realm 原型不是 Realm，迷惑。',
      error_function_tostring_not_generic: 'toString 这个方法不是万金油，用错地方了。',
      error_regex_on_non_regex: '在非正则对象上调了正则方法，离谱。',
      error_call_string_on_null: '在 null/undefined 上调 String，这不闹吗？',
      error_wrong_type: '类型错了，鸭鸭挠头。',
      error_replace_callback_not_written: '带回调的 replace 还没写，先别用。',
      error_right_hand_side_not_callable: '`__name__` 右边不可调用，嘎。',
      error_segmentation_fault: '段错误 `__offset__`，内存踩雷了。',
      error_no_arg_to_reference: '缺少参数，补上。',
      error_unimplemented_property: '这个属性类型还没实现。',
      error_unimplemented: '功能还没做，鸭鸭无奈。',
      error_cant_call_reference: '引用不能直接调用，拐个弯。',
      error_no_arg_to_dereference: '又缺参数咯，好好数数。',
      error_try_dereference_non: '想释放一个非指针？这怎么放。',
      error_no_arg_to_alloc: '`alloc` 函数要参数，你没给。',
    },
