...
#    minutes_per_class: "minutes per class"
    ...
    while_loops: "while循环"
    variables: "变量"
    basic_syntax_desc: "语法是我们编写代码的规则。语法在编写代码中很重要，就像拼写和语法在写故事和短文中那么重要。人类善于看清事物的意思，即使事物不完全正确，但计算机没那么聪明，它需要你写得非常精确。"
    while_loops_desc: "循环是一种可以在程序中重复操作的方法。您可以使用它们，来避免重复编写代码，或者当您不确定代码需要执行多少次才能完成一项任务时。"
    variables_desc: "使用变量就像在盒子中放东西一样。你给盒子起了一个名字，比如\"学校用品\"，然后你把东西放进去。盒子的内容可能会随着时间的推移而变化，但无论里面是什么，都被称为\"学校用品\"。在编程中，变量是用于存储数据的符号，这些数据可能在程序过程中发生改变。变量可以包含多种数据类型，包括数字和字符串。"
