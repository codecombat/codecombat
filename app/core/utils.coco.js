...
const isCodeCombat = true;
const isOzaria = false;
...
const combineAncestralObject = function(obj, propertyName) {
  ...
  while (obj != null ? obj[propertyName] : undefined) {
    for (var key in obj[propertyName]) {
      var value = obj[propertyName][key];
...
const countryNameToCode = country => __guard__(_.find(countries, {country: (country != null ? country.toLowerCase() : undefined)}), x => x.countryCode);
...
(function() {
  ...
  let index = 1;
  for (var key of [...Array.from(orderedCourseIDs), ...Array.from(otherOrderedCourseIDs)]) {
...
var i18n = function(say, target, language, fallback) {
  ...
  for (var localeName in say.i18n) {
    var result;
    var locale = say.i18n[localeName];
...
const kindaEqual = (compare = function(l, r) {
  if (_.isObject(l) && _.isObject(r)) {
    for (var key of Array.from(_.union(Object.keys(l), Object.keys(r)))) {
...
if (typeof document !== 'undefined' && document !== null ? document.createElement : undefined) {
  ...
  replaceText = function(elems, text) {
    for (var elem of Array.from(elems)) { elem[TEXT] = text; }
    return null;
  };
}
...
const createLevelNumberMap = function(levels) {
  ...
  for (let i = 0; i < levels.length; i++) {
    var level = levels[i];
    var levelNumber = (i - practiceLevelTotalCount) + 1;
    if (level.practice) {
...
const findNextLevel = function(levels, currentIndex, needsPractice) {
  ...
  for (let i = 0, end = currentIndex, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) { if (levels[i].locked) { return -1; } }
  ...
  if (needsPractice) {
    if (levels[currentIndex].practice || ((index < levels.length) && levels[index].practice)) {
      ...
      while ((index < levels.length) && (levels[index].complete || levels[index].assessment)) {
        if (levels[index].locked) { return -1; }
        index++;
      }
    } else {
    ...
    while ((index < levels.length) && (levels[index].practice || levels[index].complete || levels[index].assessment)) {
      if (levels[index].locked) { return -1; }
      index++;
    }
  }
...
// Convert years to an age bracket
  if (!age) { return 'open'; }
  for (var bracket of Array.from(ageBrackets)) {
    if (age <= bracket.max) {
      return bracket.slug;
    }
  }
...
const bracketToAge = function(slug) {
  ...
  let asc, end;
  let asc1, end1;
  for (i = 0, end = ageBrackets.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  for (i = 0, end1 = ageBracketsChina.length, asc1 = 0 <= end1; asc1 ? i < end1 : i > end1; asc1 ? i++ : i--) {
...
const activeArenas = function() {
  ...
  return (() => {
    const result = [];
    for (var a of Array.from(arenas)) {       var middle;
...
const activeAndPastArenas = () => (() => {
  const result = [];
  for (var a of Array.from(arenas)) {     if (a.start <= new Date()) {
      result.push(_.clone(a));
    }
  }
  return result;
...
const supportEmail = 'support@codecombat.com';
