...
class CodeLanguagesObjectTreema extends TreemaNode.nodeMap.object {
  childPropertiesAvailable () {
    return (() => {
      const result = []
      for (const key of Array.from(_.keys(aceUtils.aceEditModes))) {
...
class InternationalizationNode extends TreemaNode.nodeMap.object {
  ...
  getChildSchema (key) {
    ...
    for (const i18nProperty of Array.from(this.workingSchema.props)) {
      const parentSchemaProperties = this.parent.schema.properties != null ? this.parent.schema.properties : {}
      for (const extraSchemas of [this.parent.schema.oneOf, this.parent.schema.anyOf]) {
        for (const extraSchema of Array.from(extraSchemas != null ? extraSchemas : [])) {
  ...
  childPropertiesAvailable () {
    return (() => {
      const result = []
      for (const key of Array.from(_.keys(locale))) {
...
module.exports.LatestVersionReferenceNode = (LatestVersionReferenceNode = (function () {
  LatestVersionReferenceNode = class LatestVersionReferenceNode extends TreemaNode {
    ...
    searchCallback () {
      ...
      let first = true
      for (const model of Array.from(this.collection.models)) {
