...
const expandFlattenedDelta = function (delta, left, right, schema) {
...
const objectHash = function (obj) { if (obj != null) { return (obj.name || obj.id || obj._id || JSON.stringify(_.keys(obj).sort())) } else { return 'null' } }
...
module.exports.getConflicts = function (headDeltas, pendingDeltas) {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    while ((i + offset) < paths.length) {
      ...
      for (const headMetaDelta of Array.from((headPathMap[path] || headPathMap[nextPath]))) {
        const headDelta = headMetaDelta.delta
        for (const pendingMetaDelta of Array.from((pendingPathMap[path] || pendingPathMap[nextPath]))) {
...
const groupDeltasByAffectingPaths = function (deltas) {
  const metaDeltas = []
  for (const delta of Array.from(deltas)) {
    ...
    for (const path of Array.from(conflictPaths)) {
      metaDeltas.push({
        delta,
