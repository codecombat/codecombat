...
module.exports.createContiguousDays = function (timeframeDays, skipToday, dayOffset) {
  ...
  for (let i = 0, end = timeframeDays, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
...
module.exports.createLineChart = function (containerSelector, chartLines, containerWidth) {
  ...
  return (() => {
    ...
    // continue unless line.enabled
      let lineColor
      const xRange = d3.scale.linear().range([0, width]).domain([d3.min(line.points, d => d.x), d3.max(line.points, d => d.x)])
      ...
      // Key
      const currentKeyLine = Math.floor(currentLine / 2)
      const currentKeyXOffset = margin + (currentLine % 2 ? Math.max(leftKeyWidth + margin + 40, containerWidth / 2) : 0)
      const currentKeyYOffset = margin + height + xAxisHeight + (keyHeight * currentKeyLine)
      svg.append('line')
      ...
      const what = svg.append('text')
        .attr('x', currentKeyXOffset + 40 + 10)
        .attr('y', currentKeyYOffset + ((keyHeight + 10) / 2))
        .attr('fill', line.lineColor)
        .attr('class', 'key-text')
        .text(line.description)
        .each(function (d, i) { if ((currentLine % 2) === 0) { leftKeyWidth = Math.max(leftKeyWidth, this.getComputedTextLength()) } })

      const pointRadius = line.pointRadius != null ? line.pointRadius : 2
      // Path and points
