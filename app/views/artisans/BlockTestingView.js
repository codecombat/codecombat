...
module.exports = (BlockTestingView = (function () {
  BlockTestingView = class BlockTestingView extends RootView {
    ...
    addBlockly ({ testCase, testContainer, inputAce, outputAce }) {
      const toolbox = blocklyUtils.createBlocklyToolbox({ propertyEntryGroups: tempMethods, codeLanguage: testCase.codeLanguage })
      console.log(toolbox)
      ...
      const blocklyWorkspace = Blockly.inject(blocklyDiv[0], blocklyOptions)
      ...
      const debugBlocklyDiv = $(testContainer.find('.blockly-container-rob')[0])
      const debugBlocklyWorkspace = Blockly.inject(debugBlocklyDiv[0], blocklyOptions)

      const debugDiv = testContainer.find('.rob-debug')[0]
      ...
      try {
        const newBlocklyState = codeToBlocks({ code: testCase.code, codeLanguage: testCase.codeLanguage, toolbox, blocklyState, debugDiv, debugBlocklyWorkspace, prepData })
        if (newBlocklyState) {
          console.log('got new blockly state', newBlocklyState)
      ...
      inputAce.getSession().getDocument().on('change', () => {
        console.log('edited ace to', inputAce.getValue())
        try {
          const newBlocklyState = codeToBlocks({ code: testCase.code.trim(), codeLanguage: testCase.codeLanguage, toolbox, blocklyState, debugDiv, debugBlocklyWorkspace, prepData })
          if (newBlocklyState) {
            console.log('got new blockly state', newBlocklyState)
            blocklyUtils.loadBlocklyState(newBlocklyState, blocklyWorkspace)
          }
