...
module.exports = (ArenaBalancerView = (function () {
  ArenaBalancerView = class ArenaBalancerView extends RootView {
    ...
    onVariablesChanged (e) {
      ...
      const variables = []
      for (const variable in this.variablesTreema.data) {
        const variants = []
        for (const variant of Array.from(this.variablesTreema.data[variable])) {
    ...
    submitSessions (e) {
      ...
      for (var choice of Array.from(this.choices)) {
        ...
        const code = _.template(this.solution.source, context)
        const session = {
          name: ((() => {
            const result = []
            for (variant of Array.from(choice)) {
              result.push(variant.name)
            }
            return result
          })()).join('-'),
          code
