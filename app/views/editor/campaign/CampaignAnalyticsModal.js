...
module.exports = (CampaignAnalyticsModal = (function () {
  CampaignAnalyticsModal = class CampaignAnalyticsModal extends ModalView {
    ...
    addCompletionLineGraphs () {
      ...
      return (() => {
        const result = []
        for (const level of Array.from(this.campaignCompletions.levels)) {
          var day
          const days = []
          ...
          const data = []
          for (let i = 0, end = days.length, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
            data.push({
              x: i,
              y: days[i].rate,
              c: days[i].count
    ...
    addLineGraph (containerSelector, lineData, lineColor, min, max) {
      ...
      const lines = []
      for (let i = 0, end = lineData.length - 1, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
        lines.push({
