  ...
  CampaignEditorView = class CampaignEditorView extends RootView {
    ...
    updateCampaignLevels() {
      ...
      for (let levelIndex = 0; levelIndex < this.levels.models.length; levelIndex++) {
        ...
        if (this.campaign.get('type') === 'hero') { campaignLevel.campaign = this.campaign.get('slug'); }
      ...
      return (() => {
        ...
        for (level of Array.from(_.values(campaignLevels))) {
          ...
          var model = this.levels.findWhere({original: level.original});
          ...
          if (model.hasLocalChanges()) { result.push(this.toSave.add(model)); } else {
    ...
    afterRender() {
      ...
      this.campaignView = new CampaignView({editorMode: true, supermodel: this.supermodel}, this.campaignHandle);
...
class LevelNode extends TreemaObjectNode {
  ...
  buildValueForDisplay(valEl, data) {
    ...
    if (data.adminOnly) {
      ...
      status += " (adventurer)";
    } else if (data.releasePhase === 'beta') {
      status += " (beta)";
      el = 'span';
    }
...
class NextLevelNode extends LevelNode {
  populateData() {
    if (this.data.name != null) { return; }
    const data = _.pick(LevelsNode.levels[this.keyForParent].attributes, ['original', 'name', 'slug', 'type', 'permissions']);
    return _.extend(this.data, data);
  }
}
