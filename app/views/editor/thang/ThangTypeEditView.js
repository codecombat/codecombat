...
module.exports = (ThangTypeEditView = (function () {
  ThangTypeEditView = class ThangTypeEditView extends RootView {
    ...
    updateSpriteSheetSize () {
      let memoryMax = 0
      for (const s of Array.from(this.layerAdapter.spriteSheet._images)) { memoryMax += ((4 * s.width * s.height) / 1024 / 1024) }
      let memoryMin = 0
      for (const frame of Array.from(this.layerAdapter.spriteSheet._frames)) { memoryMin += ((4 * frame.rect.width * frame.rect.height) / 1024 / 1024) }
    ...
    onNewSpriteSheet () {
      $('#spritesheets').empty()
      for (const image of Array.from(this.layerAdapter.spriteSheet._images)) {
...
`)
      ...
      for (const [key, container] of Array.from(Object.entries(data.raw.containers))) {
        if (dryRun) {
          ...
          if ((container.c.length === 1) && (container.b[2] === boundsWidthToFix)) {
            const reference = container.c[0]
            if (data.raw.shapes[reference]) {
              const shape = data.raw.shapes[reference]
