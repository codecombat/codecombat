...
module.exports = (UnnamedView = (function () {
  UnnamedView = class UnnamedView extends ModalView {
    ...
    getRenderData () {
      ...
      c.components = ((() => {
        const result = []
        for (comp of Array.from(this.components.models)) {
          var needle
      ...
      for (var system in c.components) {
        const componentList = c.components[system]
        c.components[system] = _.sortBy(componentList, comp => comp.get('name'))
        c.nameLists[system] = ((() => {
          const result1 = []
          for (comp of Array.from(c.components[system])) {
            result1.push(comp.get('name'))
          }
          return result1
        })()).join(', ')
      }
