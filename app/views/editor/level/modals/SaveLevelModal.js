...
module.exports = (SaveLevelModal = (function () {
  SaveLevelModal = class SaveLevelModal extends SaveVersionModal {
    ...
    afterRender () {
      ...
      models = ((() => {
        const result = []
        for (const m of Array.from(models)) {
          if (m.hasWriteAccess()) {
            result.push(m)
          }
        }
    ...
    commitLevel (e) {
      ...
      for (form of Array.from(this.$el.find('form'))) {
        ...
        if (isLevelForm) {
          ...
          const [kind, klass] = Array.from($(form).hasClass('component-form') ? ['component', LevelComponent] : ['system', LevelSystem])
      ...
      for (model of Array.from(modelsToSave)) {
        ...
        if (errors = model.getValidationErrors()) {
          let messages = (Array.from(errors).map((error) => `\t ${error.dataPath}: ${error.message}`))
