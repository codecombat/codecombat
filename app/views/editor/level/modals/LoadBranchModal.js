...
module.exports = (LoadBranchModal = (function () {
  LoadBranchModal = class LoadBranchModal extends ModalView {
    ...
    initialize ({ components, systems }) {
      ...
      return this.branches.fetch({ url: '/db/branches' })
        .then(() => {
          ...
          const fetches = []
          for (const branch of Array.from(this.branches.models)) {
            for (const patch of Array.from(branch.get('patches'))) {
              const collection = patch.target.collection === 'level_component' ? this.components : this.systems
              let model = collection.get(patch.target.id)
              if (!model) {
          ...
          return Array.from(this.branches.models).map((branch) =>
            (() => {
              ...
              for (var patch of Array.from(branch.get('patches'))) {
                ...
                const originalChange = collection.get(patch.target.id).clone(false)
                ...
                const currentModel = collection.find(model => _.all([
                  model.get('original') === patch.target.original,
                  model.get('version').isLatestMajor
                ]))
                const postLoadChange = currentModel.clone()
                postLoadChange.markToRevert() // includes whatever local changes we have now

                const toApply = currentModel.clone(false)
                const applied = modelDeltas.applyDelta(toApply, patch.delta)
                if (applied) {
                  postLoadChange.set(toApply.attributes)
                  for (const key of Array.from(postLoadChange.keys())) {
                    if (!toApply.has(key)) {
    ...
    renderSelectedBranch () {
      ...
      if (this.selectedBranchDeltaViews) { for (const view of Array.from(this.selectedBranchDeltaViews)) { this.removeSubView(view) } }
      this.selectedBranchDeltaViews = []
      this.renderSelectors('#selected-branch-col')
      if (!this.selectedBranch) { return }
      return (() => {
        const result = []
        for (const patch of Array.from(this.selectedBranch.get('patches'))) {
          const originalChangeEl = this.$(`.changes-stub[data-patch-id='${patch.id}'][data-prop='original-change']`)
          this.insertDeltaView(patch.originalChange, originalChangeEl)
          const postLoadChangeEl = this.$(`.changes-stub[data-patch-id='${patch.id}'][data-prop='post-load-change']`)
    ...
    loadBranch ({ deleteBranch }) {
      ...
      for (const patch of Array.from(branch.get('patches'))) {
        ...
        const { currentModel, postLoadChange } = patch

        currentModel.set(postLoadChange.attributes)
        for (const key of Array.from(currentModel.keys())) {
