...
module.exports = (GenerateTerrainModal = (function () {
  GenerateTerrainModal = class GenerateTerrainModal extends ModalView {
    ...
    generateDecorations (preset, presetSize) {
      return (() => {
        ...
        for (var name in preset.decorations) {
          ...
          result.push((() => {
            const result1 = []
            for (const num of Array.from(_.range(presetSize.sizeFactor * _.random(decoration.num[0], decoration.num[1])))) {
              var rect
    ...
    addThang (thang) {
      ...
      for (const existingThang of Array.from(this.thangs)) {
