...
module.exports = (SettingsTabView = (function () {
  SettingsTabView = class SettingsTabView extends CocoView {
    ...
    onSettingsChanged (e) {
      ...
      for (const key of Array.from(this.editableSettings)) {
...
class SolutionGearNode extends TreemaArrayNode {
  select () {
    ...
    for (const match of Array.from((solution.source != null ? solution.source : '').match(/hero\.([a-z][A-Za-z0-9]*)/g))) {
      prop = match.split('.')[1]
    ...
    const description = 'Solution used properties: ' + [(() => {
      const result = []
      for (prop of Array.from(propertiesUsed)) {
        result.push(`<code>${prop}</code>`)
      }
      return result
    })()].join(' ')
...
class SolutionStatsNode extends TreemaNode.nodeMap.number {
  select () {
    ...
    return ThangType.calculateStatsForHeroConfig(solution.heroConfig, stats => {
      for (const key in stats) { const val = stats[key]; if (parseInt(val) !== val) { stats[key] = val.toFixed(2) } }
