...
module.exports = (ComponentsTabView = (function () {
  ComponentsTabView = class ComponentsTabView extends CocoView {
    ...
    refreshLevelThangsTreema (thangsData) {
      ...
      for (const thang of Array.from(thangsData)) {
        var left
        const componentMap = {}
      ...
      for (const comp of Array.from(componentModels)) { componentModelMap[comp.get('original')] = comp }
      let components = ((() => {
        const result = []
      ...
      components = components.concat(((() => {
        const result1 = []
        for (const c of Array.from(componentModels)) {
      ...
      const res = {}
      for (key = 0, end = treemaData.length, asc = end >= 0; asc ? key < end : key > end; asc ? key++ : key--) { res[treemaData[key].original] = treemaData[key] }
      treemaData = ((() => {
        const result2 = []
        for (key in res) {
          value = res[key]
