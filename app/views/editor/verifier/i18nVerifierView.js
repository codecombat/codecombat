...
const I18nVerifierComponent = Vue.extend({
  ...
  watch: {
    ...
    selectedLevelSlugs() {
      this.loading = true;
      const promises = [];
      for (var slug of Array.from(this.selectedLevelSlugs)) {
  ...
  methods: {
    ...
    setupRegexes() {
      ...
      for (translationKey of Array.from(translationKeys)) {
        ...
        var regex = Problem.prototype.makeTranslationRegex(englishString);
      ...
      return (() => {
        const result = [];
        for (translationKey of Array.from(translationKeys)) {
          var otherString = (otherLang.esper != null ? otherLang.esper[translationKey] : undefined) || '';
          var otherRegex = Problem.prototype.makeTranslationRegex(otherString);
    ...
    getProblems: co.wrap(function*(levelSlug) {
      ...
      for (var problem of Array.from(newProblems)) {
    ...
    compareStrings(problems) {
      return $.i18n.changeLanguage(this.language, () => {
        return problems.forEach(problem => {
          ...
          for (var regex of Array.from(this.otherRegexes)) {
