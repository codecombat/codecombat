  ...
  TeacherCourseSolutionView = class TeacherCourseSolutionView extends RootView {
    ...
    hideWrongLanguage (s) {
      ...
      return s.replace(/```([a-z]+)[^`]+```/gm, (a, l) => {
        if (['cpp', 'java', 'python', 'lua', 'coffeescript'].includes(this.language) && (l === 'javascript') && !new RegExp(`\`\`\`${this.language}`).test(s)) {
    ...
    updateLevelData () {
      ...
      for (level of Array.from((this.levels != null ? this.levels.models : undefined))) {
        ...
        if (programmableMethod) {
          ...
          const finalDefaultCode = playerCodeTag || translatedDefaultCode
