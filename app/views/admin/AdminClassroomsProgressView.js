...
module.exports = (AdminClassroomsProgressView = (function () {
  AdminClassroomsProgressView = class AdminClassroomsProgressView extends RootView {
      ...
      return Promise.all([
        ...
        .then(results => {
          ...
          if (utils.isOzaria) {
            ...
            excludedCourseIds = ((() => {
              const result = []
              for (course of Array.from(courses.filter(c => c.releasePhase !== 'released'))) {
            ...
            excludedCourseIds = ((() => {
              const result1 = []
              for (course of Array.from(courses.filter(c => c.free || (c.releasePhase !== 'released')))) {
                result1.push(course._id)
              }
              return result1
            })())
          }
          ...
          for (const prepaid of Array.from(prepaids)) {
            if (!adminMap[prepaid.creator.toString()]) {
              for (const student of Array.from(prepaid.redeemers || [])) { studentPrepaidMap[student.userID.toString()] = true }
            }
          }
          ...
          let levels = ((() => {
            const result2 = []
            for (const original in levelOriginalStringsMap) {
          ...
          let studentIds = []
          for (classroom of Array.from(this.classrooms)) {
            for (const studentId of Array.from(classroom.members)) {
          ...
          var fetchLevelSessions = (i, results) => {
            ...
            while (((i * batchSize) < studentIds.length) && (levelSessionPromises.length < 4)) {
              ...
              const end = Math.min((i * batchSize) + batchSize, studentIds.length)
              const lsPromise = api.levelSessions.getByStudentsAndLevels({ licenseLimit: this.licenseLimit, earliestCreated: this.startDay, studentIds: studentIds.slice(start, end), levelOriginals: levels, project })
          ...
          return fetchLevelSessions(0, [])
            .then(levelSessions => {
              ...
              for (var classroomId in classroomLicenseCourseLevelMap) { // when classroomId is ozar:'5d8e78879d631500344dda0c', coco:'573ac4b48edc9c1f009cd6be'
                const licensesCourseLevelMap = classroomLicenseCourseLevelMap[classroomId]
                classroom = _.find(this.classrooms, c => c._id === classroomId)
                const classroomLicenses = []

                for (var licenseId in licensesCourseLevelMap) {
                  // Build full level list and individual course indexes
                  var courseId
                  const courseLevelMap = licensesCourseLevelMap[licenseId]
                  const courseLastLevelIndexes = []
                  const courseLastLevelIndexMap = {}
                  levels = []
                  for (courseId in courseLevelMap) {
                    const levelMap = courseLevelMap[courseId]
                    for (const levelOriginal in levelMap) {
                      const data = levelMap[levelOriginal]
                  ...
                  const furthestLevelIndex = levels.indexOf(_.findLast(levels, l => l.numUsers > 0))
                  const percentComplete = ((furthestLevelIndex + 1) / levels.length) * 100
                  ...
                  for (courseId in this.latestCourseMap) { // when @latestCourseMap[courseId].slug is 'computer-science-3'
                    const courseData = this.latestCourseMap[courseId]
                  ...
                  const license = _.find(licenses, l => l._id === licenseId)
                  const courseProgressDates = classroomLicenseCourseProgressMap[classroomId][licenseId]
    ...
    addAvailableCourseMissingLevels (classroomId, classroomLicenseFurthestLevelMap, courseId, courseLastLevelIndexes, courseLevelMap, latestCourseLevelOriginals, latestOrderedLevelOriginals, levels, licenseId) {
      ...
      const currentCourseLevelOriginals = ((() => {
        ...
        for (levelOriginal in courseLevelMap[courseId]) {
          const val = courseLevelMap[courseId][levelOriginal]
      ...
      return (() => {
        ...
        for (let i = 0; i < latestLevelsToAdd.length; i++) { // when @latestCourseMap[courseId].slug is 'computer-science-4'
          ...
          const previousLatestOriginal = latestOrderedLevelOriginals[latestOrderedLevelOriginals.indexOf(levelOriginal) - 1]
          ...
          result1.push((() => {
            const result2 = []
            for (const courseLastLevelIndexData of Array.from(courseLastLevelIndexes)) {
    ...
    getClassroomActivity (classrooms, latestCourseMap, userLatestActivityMap, userLicensesMap, userLevelOriginalCompleteMap, userLevelOriginalStartedMap) {
      ...
      for (const classroom of Array.from(classrooms)) { // when classroom._id is '573ac4b48edc9c1f009cd6be'
        for (const license of Array.from(userLicensesMap[classroom.ownerID])) {
          var course, level, userId
          const licensedMembers = _.intersection(classroom.members, _.map(license.redeemers, 'userID'))
          ...
          const userFurthestLevelOriginalMap = {}
          for (userId in userLevelOriginalCompleteMap) {
            const levelOriginalCompleteMap = userLevelOriginalCompleteMap[userId]
            if (licensedMembers.indexOf(userId) >= 0) {
              if (userFurthestLevelOriginalMap[userId] == null) { userFurthestLevelOriginalMap[userId] = {} }
              for (const levelOriginal in levelOriginalCompleteMap) {
                const complete = levelOriginalCompleteMap[levelOriginal]
          ...
          for (course of Array.from(utils.sortCourses(classroom.courses))) {
            if (latestCourseMap[course._id]) {
              ...
              const levelCompleteDates = ((() => {
                const result = []
                for (const levelId in levelCompleteDateMap) {
                  const date = levelCompleteDateMap[levelId]
                  result.push(date)
                }
                return result
              })())
              ...
              levelCompleteDates.reduce((sum, currentDate) => {
                ...
                for (const target of Array.from(this.targetPercentCompleted)) {
    ...
    getLatestLevels (campaigns, courses) {
      ...
      for (const course of Array.from(courses)) {
        const campaign = _.find(campaigns, { _id: course.campaignID })
        courseLevelsMap[course._id] = { slug: course.slug, levels: [] }
        for (const levelOriginal in campaign.levels) {
          const level = campaign.levels[levelOriginal]
    ...
    getUserActivity (levelSessions, licenses, latestOrderedLevelOriginals) {
      ...
      for (const levelSession of Array.from(levelSessions)) {
      ...
      for (const license of Array.from(licenses)) {
    ...
    sortClassroomProgress (classroomProgress) {
      ...
      for (const progress of Array.from(classroomProgress)) {
        const teacherId = progress.classroom.ownerID
