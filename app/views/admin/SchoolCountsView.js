    ...
    loadData () {
      ...
      return Promise.all([
        ...
        .then((...args) => {
          ...
          for (const courseInstance of Array.from(courseInstances)) {
          ...
          for (const studentId of Array.from(studentNonHocMap)) { delete studentNonHocMap[studentId] } // Don't need these anymore
          ...
          for (const trialRequest of Array.from(trialRequests)) {
            ...
            if (props.nces_id && props.country && props.state) {
              ...
              if (country === 'USA') {
                var left
                const stateName = utils.usStateCodes.sanitizeStateName(state)
          ...
          for (country in countryStateDistrictSchoolCountsMap) {
            const stateDistrictSchoolCountsMap = countryStateDistrictSchoolCountsMap[country]
            ...
            for (state in stateDistrictSchoolCountsMap) {
              const districtSchoolCountsMap = stateDistrictSchoolCountsMap[state]
              if ((utils.usStateCodes.sanitizeStateCode(state) != null) || (['GU', 'PR'].indexOf(state) >= 0)) {
                this.countryGraphs[country].totalStates++
              }
              const stateData = { state, districts: 0, schools: 0, students: 0, teachers: 0 }
              for (district in districtSchoolCountsMap) {
                const schoolCountsMap = districtSchoolCountsMap[district]
                stateData.districts++
                const districtData = { state, district, schools: 0, students: 0, teachers: 0 }
                for (school in schoolCountsMap) {
                  const counts = schoolCountsMap[school]
                  const studentCount = Object.keys(counts.students).length
                  const teacherCount = Object.keys(counts.teachers).length
