...
module.exports = (CodePlaybackView = (function () {
  CodePlaybackView = class CodePlaybackView extends CocoView {
    ...
    afterRender () {
      ...
      return (() => {
        const result = []
        for (const ev of Array.from(this.options.events)) {
          const div = $('<div></div>')
          div.addClass('event')
          const percent = (ev.timestamp / this.maxTime) * 100
          const offset = (15 * ev.timestamp) / this.maxTime
          if (ev.eventName) {
    ...
    updateSlider () {
      ...
      return (() => {
        const result = []
        for (let child of Array.from(this.$el.find('#event-container').children())) {
          child = $(child)
          const timeoutValue = child.data('timeout') || 0
          if (!(timeoutValue >= 0)) { continue }
          const percentage = timeoutValue / 100
