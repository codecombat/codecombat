...
module.exports = (AdminClassroomContentView = (function() {
  AdminClassroomContentView = class AdminClassroomContentView extends RootView {
    ...
    fetchData(userIds) {
      ...
      const userIdMap = {};
      for (var id of Array.from((userIds || []))) { userIdMap[id] = true; }
      var getMoreLevelSessions = (courseIDs, startOffset, endOffset) => {
        ...
        for (courseID of Array.from(courseIDs)) {
          var url = `/db/classroom/-/playtimes?sessionLimit=${this.minSessionCount * 100}&courseID=${courseID}&startDay=${encodeURIComponent(startDay)}`;
        ...
        .then(results => {
          ...
          for (let index = 0; index < results.length; index++) {
            ...
            for (var levelPlaytime of Array.from(levelPlaytimes)) {
            ...
            for (var session of Array.from(levelSessions)) {
          ...
          for (courseID in courseLevelTotalPlaytimeMap) {
            var totalPlaytimes = courseLevelTotalPlaytimeMap[courseID];
          ...
          if ((startOffset <= this.maxDays) && (courseIDs.length > 0)) {
            ...
            for (data of Array.from(this.courseLevelPlaytimes)) {
              ...
              var avgPlaytime = data.count > 0 ? data.playtime / data.count : 300;
            ...
            this.courseSeconds = ((() => {
              const result1 = [];
              for (var courseSlug in courseSecondsMap) {
      ...
      return getMoreLevelSessions(((() => {
        const result = [];
        for (var key in utils.courseIDs) {
