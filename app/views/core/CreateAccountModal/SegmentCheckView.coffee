  ...
  onSubmitSegmentCheck: (e) ->
    ...
    if @signupState.get('path') is 'student'
      ...
      if _.isNaN(@signupState.get('birthday').getTime())
        ...
        if age > 13
          @trigger 'nav-forward'
          window.tracker?.trackEvent 'CreateAccountModal Teacher SegmentCheckView Submit', category: 'Individuals'
        else
          @trigger 'nav-forward', 'coppa-deny'
          window.tracker?.trackEvent 'CreateAccountModal Teacher SegmentCheckView Coppa Deny', category: 'Individuals'
