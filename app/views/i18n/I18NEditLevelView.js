  ...
  I18NEditLevelView = class I18NEditLevelView extends I18NEditModelView {
    ...
    buildTranslationList () {
      ...
      for (let gIndex = 0; gIndex < iterable1.length; gIndex++) {
        const goals = iterable1[gIndex]
        const iterable2 = goals.goals != null ? goals.goals : []
      ...
      for (let scriptIndex = 0; scriptIndex < iterable6.length; scriptIndex++) {
        const script = iterable6[scriptIndex]
        const iterable7 = script.noteChain != null ? script.noteChain : []
        for (let noteGroupIndex = 0; noteGroupIndex < iterable7.length; noteGroupIndex++) {
          const noteGroup = iterable7[noteGroupIndex]
          if (!noteGroup) { continue }
          const iterable8 = noteGroup.sprites != null ? noteGroup.sprites : []
          for (let spriteCommandIndex = 0; spriteCommandIndex < iterable8.length; spriteCommandIndex++) {
            const spriteCommand = iterable8[spriteCommandIndex]
            ...
            const iterable9 = (spriteCommand.say != null ? spriteCommand.say.responses : undefined) != null ? (spriteCommand.say != null ? spriteCommand.say.responses : undefined) : []
            for (let responseIndex = 0; responseIndex < iterable9.length; responseIndex++) {
              const response = iterable9[responseIndex]
