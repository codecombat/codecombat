...
module.exports = class LeaderboardView extends CocoView
  ...
  constructor: (options, @level, @sessions, @anonymousPlayerName) ->
    ...
    @propsData = { @tableTitles, @league, @level, @leagueType, @course, scoreType: 'tournament', showContactUs: @anonymousPlayerName && me.isTeacher() }
    unless @tournament
  ...
  mapFullName: (fullName) ->
    fullName = fullName?.replace(/^Anonymous/, $.i18n.t('general.player'))
    fullName = fullName?.replace(/^AIAlgorithm_(.+)_$/, '$1')
    fullName = fullName?.replace(/^AIYouth_(.+)_$/, '$1')
    fullName
