...
module.exports = (ClanDetailsView = (function () {
  ClanDetailsView = class ClanDetailsView extends RootView {
        ...
        for (const campaign of Array.from(this.campaigns.models)) {
          if (campaign.get('type') === 'hero') {
            ...
            for (const levelID in object) {
              ...
              for (const member of Array.from((this.members != null ? this.members.models : undefined) != null ? (this.members != null ? this.members.models : undefined) : [])) {
                if (context.memberLevelStateMap[member.id] != null ? context.memberLevelStateMap[member.id][levelSlug] : undefined) {
                  ...
                  if (level.concepts != null) {
                    ...
                    for (const concept of Array.from(level.concepts)) {
    ...
    onMembersSync () {
      this.stats.averageRank = Math.round(this.members.reduce((sum, member) => sum + member.rank(), 0) / this.members.length)
      return (typeof this.render === 'function' ? this.render() : undefined)
    }
    ...
    onMemberAchievementsSync () {
      ...
      for (const achievement of Array.from(this.memberAchievements.models)) {
    ...
    onLaddersSync (e) {
      const levels = []
      for (const ladder of Array.from(this.ladders.models)) {
        levels.push({
