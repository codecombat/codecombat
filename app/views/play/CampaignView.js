...
module.exports = (CampaignView = (function () {
  CampaignView = class CampaignView extends RootView {
    ...
    generateCompletionRates () {
      ...
      return (() => {
        const result = []
        for (const level of Array.from((this.campaign != null ? this.campaign.renderedLevels : undefined) != null ? (this.campaign != null ? this.campaign.renderedLevels : undefined) : [])) {
    ...
    onLevelCompletionsLoaded (level, data) {
      ...
      $(`div[data-level-slug=${level.slug}] .level-kind`).css({ color, width: 256 + 'px', transform: 'translateX(-50%) translateX(15px)' })
...
`
        ...
        for (const otherLevel of Array.from(orderedLevels)) {
          if (!level.unlockedInSameCampaign && (otherLevel !== level)) {
            for (const reward of Array.from((otherLevel.rewards != null ? otherLevel.rewards : []))) {
              if (reward.level) {
    ...
    createLines () {
      return Array.from((this.campaign != null ? this.campaign.renderedLevels : undefined) != null ? (this.campaign != null ? this.campaign.renderedLevels : undefined) : []).map((level) =>
        (() => {
          const result = []
          for (const nextLevelOriginal of Array.from(level.nextLevels != null ? level.nextLevels : [])) {
            var nextLevel
      ...
      return achievements.fetchForCampaign(
        ...
        .done(achievements => {
          ...
          for (var achievement of Array.from(levelAchievements)) {
            ...
            for (const levelOriginal of Array.from(achievement.rewards.levels)) {
