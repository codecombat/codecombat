  ...
  AnnouncementModal = class AnnouncementModal extends ModalView {
    ...
    onClickRiticBlock (e) {
      ...
      const spawnSomeShards = num => (() => {
        const result = []
        for (let i = 0, end = num, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
          const img = $('<img>').attr('src', `/images/pages/play/modal/announcement/ritic/shard${Math.floor(1 + (Math.random() * 6))}.png`).addClass('shard')
          const left = Math.floor(25 + (50 * Math.random()))
          const top = Math.floor(75 - (30 * Math.random()))
          img.css({ left: left + '%', top: top + '%' })
          img.css('transform', 'rotate(0deg)')
          $('#ice-chamber').append(img)
          const randNum = Math.random() * Math.PI * 2
          img.animate({
            opacity: 0,
