  ...
  LevelPlaybackView = class LevelPlaybackView extends CocoView {
    static initClass() {
      ...
      this.prototype.events = {
        ...
        'click #zoom-out-button'() { if (!this.shouldIgnore()) { return Backbone.Mediator.publish('camera:zoom-out', {}); } },
    ...
    afterRender() {
      ...
      if (this.timePopup == null) { this.timePopup = new HoverPopup; }
    ...
    updatePopupContent() {
      return (this.timePopup != null ? this.timePopup.updateContent(`<h2>${this.timeToString(this.newTime)}</h2>${this.formatTime(this.current, this.currentTime)}<br/>${this.formatTime(this.total, this.totalTime)}`) : undefined);
    }
    ...
    updateBarWidth(loadedFrameCount, maxTotalFrames, dt) {
      ...
      const pct = parseInt((100 * loadedFrameCount) / (maxTotalFrames - 1)) + '%';
    ...
    onSetPlaying(e) {
      ...
      if (!this.options.level.isType('game-dev')) { this.playSound((playing ? 'playback-play' : 'playback-pause')); }
    ...
    onFrameChanged(e) {
      if (e.progress !== this.lastProgress) {
        ...
        if (this.timePopup != null ? this.timePopup.shown : undefined) { this.updatePopupContent(); }
    ...
    onProgressEnter(e) {
      ...
      return (this.timePopup != null ? this.timePopup.enter(this.timePopup) : undefined);
    }

    onProgressLeave(e) {
      return (this.timePopup != null ? this.timePopup.leave(this.timePopup) : undefined);
    }

    onProgressHover(e, offsetX) {
      const timeRatio = this.$progressScrubber.width() / this.totalTime;
      if (offsetX == null) { offsetX = e.clientX - $(e.target).closest('#timeProgress').offset().left; }
      offsetX = Math.max(0, offsetX);
      this.newTime = offsetX / timeRatio;
      this.updatePopupContent();
      if (this.timePopup != null) {
        this.timePopup.onHover(e);
      }
    ...
    hookUpScrubber() {
      ...
      return this.$progressScrubber.slider({
        ...
        slide: (event, ui) => {
          ...
          this.scrubTo(ui.value / this.sliderIncrements);
          if (ratioChange = this.getScrubRatio() - oldRatio) {
            const sound = `playback-scrub-slide-${ratioChange > 0 ? 'forward' : 'back'}-${this.slideCount % 3}`;
            if (!/back/.test(sound)) {  // We don't have the back sounds in yet: http://discourse.codecombat.com/t/bug-some-mp3-lost/4830
              return this.playSound(sound, (Math.min(1, Math.abs(ratioChange * 50))));
            }
          }
        },
