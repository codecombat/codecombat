...
module.exports = (CourseRewardsView = (function () {
  CourseRewardsView = class CourseRewardsView extends CocoView {
    ...
    loadAchievementsData () {
      ...
      for (const itemOriginal of Array.from(itemOriginals)) {
    ...
    getRenderData () {
      ...
      for (achievement of Array.from(((this.achievements != null ? this.achievements.models : undefined) || []))) {
        const earnedAchievement = earnedAchievementMap[achievement.id]
      ...
      for (achievement of Array.from(c.achievements)) {
        ...
        if (!this.supermodel.finished() || !proportionalTo) { continue }
        // For repeatable achievements, we modify their base worth/gems by their repeatable growth functions.
        const achievedAmount = utils.getByPath(this.session.attributes, proportionalTo)
        const previousAmount = Math.max(0, achievedAmount - 1)
        const func = achievement.getExpFunction()
    ...
    beginSequentialAnimations () {
      ...
      this.sequentialAnimationInterval = setInterval(this.tickSequentialAnimation, 1000 / 60)
    }
    ...
    tickSequentialAnimation () {
      ...
      if (panel.unit === 'xp') {
        ...
        if (ratio > 0.5 && ratio < 0.6) { this.playSound('item-unlocked') }
      }
      if (ratio === 1) {
