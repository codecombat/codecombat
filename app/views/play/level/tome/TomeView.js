...
module.exports = (TomeView = (function() {
  TomeView = class TomeView extends CocoView {
    ...
    afterRender() {
      ...
      delete this.options.thangs;
      this.$el.toggleClass('blocks', Boolean(this.blocks));
      return (this.options.playLevelView != null ? this.options.playLevelView.$el.toggleClass('blocks', Boolean(this.blocks)) : undefined);
    }
    ...
    determineBlocksSettings() {
      let left, left1;
      let left2;
      const language = (left = (left1 = this.options.session.get('codeLanguage')) != null ? left1 : __guard__(me.get('aceConfig'), x => x.language)) != null ? left : 'python';
      if (this.options.level.isType('web-dev', 'game-dev') || (!['python', 'javascript', 'lua'].includes(language))) {
        // TODO: eventually we could get game-dev working, if we figure out how to list spawnables
        this.blocks = false;
        this.blocksHidden = true;
        return;
      }
      const classroomBlocks = (this.options.classroomAceConfig != null ? this.options.classroomAceConfig.blocks : undefined) != null ? (this.options.classroomAceConfig != null ? this.options.classroomAceConfig.blocks : undefined) : 'opt-in';
      this.blocksHidden = false;
      switch (classroomBlocks) {
        case 'hidden': return this.blocksHidden = true;
        case 'opt-in': return this.blocks = __guard__(me.get('aceConfig'), x1 => x1.blocks);
        case 'opt-out': return this.blocks = (left2 = __guard__(me.get('aceConfig'), x2 => x2.blocks)) != null ? left2 : true;
      }
    }
      ...
      for (thang of Array.from(programmableThangs)) {
        ...
        for (var methodName in thang.programmableMethods) {
          ...
          spell = (this.spells[spellKey] = new Spell({
            ...
            blocks: this.blocks,
            blocksHidden: this.blocksHidden,
    ...
    onToggleBlocks(e) {
      let left;
      if (Boolean(this.blocks) === Boolean(e.blocks)) { return; }
      this.blocks = Boolean(e.blocks);
      this.$el.toggleClass('blocks', this.blocks);
      if (this.options.playLevelView != null) {
        this.options.playLevelView.$el.toggleClass('blocks', this.blocks);
      }
      $(window).trigger('resize');
      const aceConfig = (left = me.get('aceConfig')) != null ? left : {};
      if (aceConfig.blocks !== this.blocks) {
        aceConfig.blocks = this.blocks;
