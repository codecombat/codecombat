...
module.exports = (ProblemAlertView = (function () {
  ProblemAlertView = class ProblemAlertView extends CocoView {
    ...
    setProblemMessage () {
      if (this.problem != null) {
        ...
        if ((this.problem.hint != null) && /(?:null|undefined)/.test(this.problem.hint)) {
          const age = this.problem.userInfo?.age
          if (age != null) {
    ...
    onShowProblemAlert (data) {
      ...
      return application.tracker?.trackEvent('Show problem alert', { levelID: this.level.get('slug'), ls: this.session?.get('_id') })
    }
    ...
    onJiggleProblemAlert () {
      ...
      const pauseJiggle = () => {
        return this.$el?.removeClass('jiggling')
      }
