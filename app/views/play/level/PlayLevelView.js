...
module.exports = (PlayLevelView = (function() {
  PlayLevelView = class PlayLevelView extends RootView {
    ...
    load() {
      ...
      const levelLoaderOptions = { supermodel: this.supermodel, levelID: this.levelID, sessionID: this.sessionID, opponentSessionID: this.opponentSessionID, team: utils.getQueryVariable('team'), observing: this.observing, courseID: this.courseID, courseInstanceID: this.courseInstanceID };
      if (me.isSessionless()) {
      ...
      if (this.courseInstanceID) {
        ...
        return fetchAceConfig.then(classroom => {
          return this.classroomAceConfig.liveCompletion = (classroom.aceConfig != null ? classroom.aceConfig.liveCompletion : undefined) != null ? (classroom.aceConfig != null ? classroom.aceConfig.liveCompletion : undefined) : true;
        });
      }
    }
    ...
    insertSubviews() {
      ...
      if (me.isAdmin()) {
        this.insertSubView(new ChatView({levelID: this.levelID, sessionID: this.session.id, session: this.session}));
      }
