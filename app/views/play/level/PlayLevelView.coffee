...
module.exports = class PlayLevelView extends RootView
  ...
  load: ->
    ...
    if @courseInstanceID
      fetchACE = $.get("/db/course_instance/#{@courseInstanceID}/classroom?project=aceConfig,members")
      @supermodel.trackRequest fetchACE
      fetchACE.then (classroom) =>
        @classroomACE = classroom.aceConfig
  ...
  insertSubviews: ->
    ...
    @insertSubView @tome = new TomeView { @levelID, @session, @otherSession, playLevelView: @, thangs: @world?.thangs ? [], @supermodel, @level, @observing, @courseID, @courseInstanceID, @god, @hintsState, @classroomACE }
    ...
    @insertSubView @controlBar = new ControlBarView {worldName: utils.i18n(@level.attributes, 'name'), session: @session, level: @level, supermodel: @supermodel, courseID: @courseID, courseInstanceID: @courseInstanceID, @classroomACE}
