...
module.exports = (AccountSettingsView = (function () {
  AccountSettingsView = class AccountSettingsView extends CocoView {
    ...
    getEmailSubsDict () {
      const subs = {}
      for (const sub of Array.from(this.user.getEnabledEmails())) { subs[sub] = 1 }
      return subs
    }
    ...
    getSubscriptions () {
      ...
      const inputs = ((() => {
        const result = []
        for (i of Array.from($('#email-panel input[type="checkbox"].changed', this.$el))) {
          result.push($(i))
        }
        return result
      })())
      const emailNames = ((() => {
        const result1 = []
        for (i of Array.from(inputs)) {
          result1.push(i.attr('name').replace('email_', ''))
        }
        return result1
      })())
      const enableds = ((() => {
    ...
    grabOtherData () {
      ...
      if (features.chinaInfra && (features.chinaHome || me.isAdmin() || (window.serverSession && window.serverSession.amActually))) {
      ...
      for (const emailName in object) {
        const enabled = object[emailName]
