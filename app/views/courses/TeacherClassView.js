  ...
  TeacherClassView = class TeacherClassView extends RootView {
    ...
    attachMediatorEvents () {
      ...
      this.listenTo(this.classroom, 'sync change update', function () {
        ...
        for (const course of Array.from(this.sortedCourses)) { this.availableCourseMap[course._id] = true }
      ...
      this.listenTo(this.students, 'sync change update add remove reset', function () {
        ...
        for (const student of Array.from(this.students.models)) {
    ...
    onClickExportStudentProgress () {
      ...
      const courses = ((() => {
        const result = []
        for (c of Array.from(this.sortedCourses)) {
          result.push(this.courses.get(c._id))
        }
        return result
      })())
      ...
      for (const student of Array.from(this.students.models)) {
        ...
        let levels = 0
        let playtime = 0
        for (const session of Array.from(this.classroom.sessions.models)) {
        ...
        // Find the prepaids and users we're acting on (for both starter and full license cases)
          ...
          // These ones just matter for display
            ...
            const numFullLicensesAvailable = _.reduce(((() => {
              const result1 = []
              for (prepaid of Array.from(availableFullLicenses)) {
                result1.push(prepaid.openSpots())
              }
              return result1
            })()), (val, total) => val + total) || 0
          ...
          for (prepaid of Array.from(availablePrepaids)) {
            if (Math.min(_.size(unenrolledStudents), prepaid.openSpots()) > 0) {
              for (let i = 0, end = Math.min(_.size(unenrolledStudents), prepaid.openSpots()), asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
