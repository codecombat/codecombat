...
module.exports = (EnrollmentsView = (function () {
  EnrollmentsView = class EnrollmentsView extends RootView {
    ...
    uniqueStudentsPerYear (allClassrooms) {
      ...
      const years = {}
      for (const classroom of Array.from(allClassrooms)) {
        const { _id, members } = classroom
        if ((members != null ? members.length : undefined) > 0) {
          const creationDate = moment(dateFromObjectId(_id))
          const year = this.relativeToYear(creationDate)
          if (!years[year]) {
            years[year] = new Set(members)
          } else {
            const yearSet = years[year]
