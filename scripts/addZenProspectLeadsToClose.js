    ...
    request.get(url, (error, response, body) => {
      ...
      request.get(url, (error, response, body) => {
        ...
        if (data.total_results === 0) {
          ...
          console.error(`ERROR: ${data.total_results} leads found with info from Zen Prospect: email=${zpContact.email}, organization=${zpContact.organization}, nces_district_id=${zpContact.nces_district_id}, nces_school_id=${zpConctact.nces_school_id}, nces_district_id=${zpConctact.nces_district_id}. Final query: ${query}`);
          return done()
        }
        else {
