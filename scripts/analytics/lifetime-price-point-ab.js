...
co(function * () {
  ...
  var res = yield client.queryAsync("select \"user\", root_tstamp from atomic.com_codecombat_view_load_1 where view_id = 'subscribe-modal' and root_tstamp > '"+date+"';", [])
  ...
  startId = utils.objectIdFromTimestamp(new Date(date).getTime())
  ...
  for (var j in eventLogs) {
    ...
    if(eventLog.properties && eventLog.properties.value) {
      compiled[eventLog.user].events[eventLog.event] = eventLog.properties.value;
    } else {
  ...
  while (userIds.length) {
    ...
    _.forEach(users, function(user) {
      ...
      res.send(products)
      */
      if(user.testGroupNumber % 2 === 0) {
        compiled[user._id+''].group = 'lifetime79'
      } else {
        compiled[user._id+''].group = 'lifetime99'
      }
      //compiled[user._id+''].group = (user.testGroupNumber || 0) % 2 === 0 ? 'lifetime99' : 'lifetime79'
    })
  }
  ...
  for (var k in userIds) {
    ...
    for(var l in userEvents) {
      ...
      */
      ...
      } else if (eventName === 'Started subscription purchase') {
        counts[compiled[userId].group].subsStarted += 1
      }
      else {
