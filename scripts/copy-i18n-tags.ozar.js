...
const CHANGE_PATTERN = new RegExp(
  `\\s?\\s?(#\\s)?${escapeRegexp(CHANGE_MARKER)}`,
  'gi'
) // Identify translation marked change
...
const localeFiles = fs
  .readdirSync(path.join(__dirname, '../app/locale'))
...
for (const localeFile of localeFiles) {
  ...
  for (const enCategoryName of Object.keys(enTranslations)) {
    ...
    const categoryRegexPrefix = `\\s\\s${escapeRegexp(
      enCategoryName
    )}:\\n(?:.+\\n)*`
    ...
    for (const enTagName of Object.keys(enCategory)) {
      ...
      const sourceFileTag = QUOTE_TAG_NAME_PATTERN.test(enTagName)
        ? enTagName
        : `"${enTagName}"`
      ...
      const commentedTagRegex = new RegExp(
        categoryRegexPrefix + `#\\s+${escapeRegexp(sourceFileTag)}:`
      )
